...
import { BarChart3, ShoppingBag, Users, AlertTriangle, FileText, TrendingUp, TrendingDown, Minus, Bell, Shield, Star, ArrowLeft } from 'lucide-react';
import { Badge } from '@/components/ui/badge';
import { Button } from '@/components/ui/button';
import { sellers, alerts, reviews, products } from '@/data/mockData';
import { BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer, LineChart, Line, PieChart, Pie, Cell } from 'recharts';
import { toast } from '@/hooks/use-toast';

const trendIcon = { improving: TrendingUp, declining: TrendingDown, stable: Minus };
const trendColor = { improving: 'text-success', declining: 'text-destructive', stable: 'text-muted-foreground' };
const statusBadge = {
  active: 'bg-success/10 text-success border-success/20',
  warned: 'bg-warning/10 text-warning border-warning/20',
  suspended: 'bg-destructive/10 text-destructive border-destructive/20',
};
const severityBadge = { high: 'bg-destructive/10 text-destructive', medium: 'bg-warning/10 text-warning', low: 'bg-info/10 text-info' };
...
const AdminDashboard = () => {
  ...
  const avgSentiment = Math.round(sellers.reduce((a, s) => a + s.sentimentBreakdown.positive, 0) / sellers.length);
  const unresolvedAlerts = alerts.filter(a => !a.resolved).length;
  const pieData = [
    { name: 'Positive', value: selectedSeller.sentimentBreakdown.positive },
    { name: 'Negative', value: selectedSeller.sentimentBreakdown.negative },
    { name: 'Neutral', value: selectedSeller.sentimentBreakdown.neutral },
  ];
            ...
            <div className="grid lg:grid-cols-2 gap-6">
              <div className="stat-card">
                ...
                <ResponsiveContainer width="100%" height={280}>
                  <BarChart data={sellers.map(s => ({ name: s.name.split(' ')[0], score: s.reputationScore }))}>
                    ...
                    return (
                      <div key={s.id} className="flex items-center justify-between p-3 rounded-lg bg-secondary/30 hover:bg-secondary/50 transition-colors cursor-pointer" onClick={() => { setSelectedSeller(s); setTab('sellers'); }}>
                        <div>
                          ...
                          <p className="text-xs text-muted-foreground">{s.productCount} products · {s.totalReviews} reviews</p>
                        </div>
                        <div className="flex items-center gap-3">
                          <Badge variant="outline" className={statusBadge[s.status]}>{s.status}</Badge>
                          <div className="flex items-center gap-1">
                            <Star className="h-4 w-4 fill-warning text-warning" />
                            <span className="font-semibold text-sm text-foreground">{s.avgRating}</span>
                          </div>
              ...
              <div className="stat-card lg:col-span-2">
                <div className="flex items-center justify-between mb-4">
                  <div>
                    ...
                    <Badge variant="outline" className={statusBadge[selectedSeller.status]}>{selectedSeller.status} · {selectedSeller.warningCount} warnings</Badge>
                  </div>
                ...
                <div className="grid grid-cols-3 gap-4 mb-6">
                  <div className="text-center p-3 bg-secondary/30 rounded-lg"><p className="text-2xl font-bold text-foreground">{selectedSeller.reputationScore}</p><p className="text-xs text-muted-foreground">Reputation</p></div>
                  <div className="text-center p-3 bg-secondary/30 rounded-lg"><p className="text-2xl font-bold text-foreground">{selectedSeller.avgRating}</p><p className="text-xs text-muted-foreground">Avg Rating</p></div>
                  <div className="text-center p-3 bg-secondary/30 rounded-lg"><p className="text-2xl font-bold text-foreground">{selectedSeller.totalReviews}</p><p className="text-xs text-muted-foreground">Total Reviews</p></div>
                </div>
                <h4 className="font-medium text-foreground mb-3">Rating & Sentiment Trend</h4>
                <ResponsiveContainer width="100%" height={250}>
                  <LineChart data={selectedSeller.monthlyData}>
                    <CartesianGrid strokeDasharray="3 3" stroke="hsl(214,32%,91%)" />
              ...
              <div className="space-y-4">
                ...
                <div className="stat-card">
                  ...
                  <div className="space-y-2">
                    {selectedSeller.topComplaints.map((c, i) => (
                ...
                <div className="stat-card">
                  ...
                  <p className="text-sm text-muted-foreground leading-relaxed">
                    {selectedSeller.reputationScore < 50
                      ? `⚠️ ${selectedSeller.name} has critically low scores. Consider suspension if no improvement within 30 days. Key areas: ${selectedSeller.topComplaints.slice(0, 2).join(', ')}.`
                      : selectedSeller.trend === 'improving'
                      ? `✅ ${selectedSeller.name} is trending positively. Continue monitoring. Minor issues with ${selectedSeller.topComplaints[0]?.toLowerCase() || 'N/A'}.`
                      : `📊 ${selectedSeller.name} is stable. Watch for ${selectedSeller.topComplaints[0]?.toLowerCase() || 'potential issues'}.`
                    }
                  </p>
                </div>
              </div>
          ...
          <div className="space-y-4">
            ...
            {alerts.map(a => (
              <div key={a.id} className={`stat-card flex items-start justify-between gap-4 ${a.resolved ? 'opacity-60' : ''}`}>
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <div className="flex items-center gap-2 mb-1">
                      <span className="font-medium text-foreground text-sm">{a.sellerName}</span>
                ...
                {!a.resolved && <Button size="sm" variant="outline" onClick={() => toast({ title: 'Alert acknowledged' })}>Resolve</Button>}
              </div>
            ))}
          </div>
        )}
          ...
          <div className="space-y-6">
            ...
            <div className="stat-card">
              ...
              <div className="prose prose-sm max-w-none text-muted-foreground space-y-3">
                <p>This month, <strong className="text-foreground">12 reviews</strong> were collected across <strong className="text-foreground">8 products</strong> from <strong className="text-foreground">5 sellers</strong>. Overall sentiment is mixed with an average positive rate of {avgSentiment}%.</p>
                    ...
                    {sellers.map(s => {
                      ...
                      return (
                        <tr key={s.id} className="border-b border-border/50">
                          ...
                          <td className="py-3 text-center text-foreground">{s.avgRating}</td>
                          <td className="py-3 text-center"><span className={`font-semibold ${s.reputationScore >= 70 ? 'text-success' : s.reputationScore >= 50 ? 'text-warning' : 'text-destructive'}`}>{s.reputationScore}/100</span></td>
                          <td className="py-3 text-center text-foreground">{s.sentimentBreakdown.positive}% pos</td>
